...
def split_files(vmu: Vmu):
    new_vmus = {}
    for current_index,file in enumerate(vmu.directory.files()):
        ...
        if(len(matches) == 0):
            ...
            if(loader != "disc"):
                current_file = vmu.get_file(current_index)
...
vmuList = os.listdir(import_dir)

for vmu in vmuList:
    currentVmu = Vmu(import_dir + vmu)
